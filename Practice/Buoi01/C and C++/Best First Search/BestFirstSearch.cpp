...
void BestFirstSearch()
{
    ...
    tmax.g = tmax.dad = 0;
    int i;
    // Thêm đỉnh bắt đầu vào danh sách OPEN
    add(tmax,open,openSize);
    bool stop = false;

    while(!stop) // Vòng lặp chính, chạy đến khi tìm thấy đích
    {
        printf("\n BEST NODE: \n\t");
        display(tmax);
        // Xóa node tốt nhất khỏi OPEN và thêm vào CLOSE
        remove(tmax,open,openSize);
        add(tmax,close,closeSize);
        if (tmax.id == end) // Kiểm tra điều kiện đạt mục tiêu
        {
        ...
        {
            for (i=1; i<=n; i++) // Phát sinh các nút con
            {
                if (cost[tmax.id][i]>0) // Nếu có cạnh kề
                {
                    // Xử lý nếu nút con đã có trong OPEN
                    if (isIn(v[i],open,openSize))
                        if (tmax.g + cost[tmax.id][i] < v[i].g)
                        {
                            remove(v[i],open,openSize);
                            v[i].g = tmax.g + cost[tmax.id][i]; // Cập nhật chi phí
                            v[i].dad = tmax.id;
                            add(v[i],open,openSize);
                        }
                    // Xử lý nếu nút con chưa có trong OPEN và CLOSE
        ...
        if (openSize>0)
            tmax = Min(open,openSize); // Lấy nút tốt nhất từ OPEN cho vòng lặp tiếp theo
    ...
    Backtrack(close[closeSize],close,closeSize); // Truy vết đường đi
}
